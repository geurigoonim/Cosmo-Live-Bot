...
const commands = [
  new SlashCommandBuilder()
    .setName('live-notify')
    .setDescription('Announce that an artist is going live on COSMO')
    .addStringOption(option =>
      option.setName('artist')
        .setDescription('Name of the artist going live')
        .setRequired(true))
    .addStringOption(option =>
...
client.on('interactionCreate', async interaction => {
  ...
  if (commandName === 'live-notify') {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(`**${artistName}** is now live on COSMO!`)
      .addFields(
    ...
    try {
      ...
      await interaction.reply({ 
        content: `✅ Live notification sent for **${artistName}** in ${channel}!`, 
        ephemeral: true 
      });
    } catch (error) {
